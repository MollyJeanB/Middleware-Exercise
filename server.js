...
function gateKeeper(req, res, next) {
  ...
  const headerData = req.get("x-username-and-password");
  console.log(headerData);
  const currentUser = queryString.parse(headerData);
  const foundUser = USERS.find(element => {
    return (
      element.userName === currentUser.user &&
      currentUser.pass === element.password
    );
  });
  if (foundUser) {
    req.user = foundUser;
...
app.get("/api/users/me", (req, res) => {
  ...
  const { firstName, lastName, id, user, position } = req.user;
  return res.json({ firstName, lastName, id, user, position });
});
